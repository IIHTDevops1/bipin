...
<div class="fixed-bar">
    <div class="item-title">
        <h3>投诉列表</h3>
        <!--<ul class="tab-base">-->
            <!--<li><a href="<?php echo U('brand');?>" <?php if(empty($classification)): ?>class="current"<?php endif; ?>><span>全部分类</span></a></li>-->
...
<div class="fixed-empty"></div>
    <form method="post" name="formSearch" action="<?php echo U('tousu');?>" id="formSearch">

        <table class="tb-type1 noborder search">
            <tbody>
            <tr>
                <td><input type="text" value="<?php echo ($news_title); ?>" name="news_title" id="news_title" class="txt" placeholder="敏感词搜索"></td>
...
<form method='post' id="form_apply" action="">
    ...
    <table class="table tb-type3" id="newslist">
        ...
        <tr class="thead">
            <th class="w12">序号</th>
            <!--<th class="w60">上级分类</th>-->

            <th class="w60">用户昵称</th>
            <th class="w60">用户头像</th>
            <th class="w60">投诉作品</th>
            <th class="w60">作品发布人id</th>
            <th class="w60">投诉内容</th>
        ...
        <tbody>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$news): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><?php echo (get_username($news["user_id"])); ?></td>
                    <td><img src="<?php echo (get_thumb($news["user_id"])); ?>" style="height:50px;width: 50px;"></td>
                    <td><?php echo (get_title($news["worker_id"])); ?></td>
                    <td><?php echo (get_user($news["worker_id"])); ?></td>
                    <td><?php echo ($news["content"]); ?></td>


                    <td>
                        <!--<a class="btn" href="<?php echo U('brand_edit',array('id'=>$news['id']));?>"><span>编辑</span></a>-->
                        <a class="btn del" href="<?php echo U('del_pinglun',array('id'=>$news['id'],'proid'=>$pro));?>"><span>删除</span></a>
                    </td>
