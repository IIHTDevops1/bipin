...
<div class="fixed-bar">
    <div class="item-title">
        <h3>公告管理</h3>
        <ul class="tab-base">
            <li><a href="<?php echo U('ggadd');?>"><span>添加</span></a></li>
        </ul>
    </div>
</div>
<div class="fixed-empty"></div>
    <form method="post" name="formSearch" action="<?php echo U('gonggao');?>" id="formSearch">

        <table class="tb-type1 noborder search">
            <tbody>
            <tr>
                <th><label for="stem"> 关键字</label></th>
                <td><input type="text" value="<?php echo ($stem); ?>" name="stem" id="stem" class="txt"></td>
    ...
    <table class="table tb-type3" id="newslist">
        ...
        <tbody>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$news): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><?php echo ($news["content1"]); ?></td>
                    <td><?php echo (dateformat($news["addtime"])); ?></td>
                    <td>
                        <a class="btn" href="<?php echo U('ggadd',array('id'=>$news['id']));?>"><span>编辑</span></a>
                        <a class="btn" href="<?php echo U('common_dele',array('id'=>$news['id']));?>"><span>删除</span></a>
                    </td>
