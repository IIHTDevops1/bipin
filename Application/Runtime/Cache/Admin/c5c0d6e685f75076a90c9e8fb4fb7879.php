...
<div class="fixed-bar">
    <div class="item-title">
        <h3>vip管理</h3>
        <ul class="tab-base">
            <li><a href="<?php echo U('vips');?>" <?php if(empty($classification)): ?>class="current"<?php endif; ?>><span>全部分类</span></a></li>

            <li><a href="<?php echo U('addvip');?>"><span>添加</span></a></li>
        </ul>
    </div>
</div>
    ...
    <table class="table tb-type3" id="newslist">
        ...
        <tbody>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$news): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><?php echo ($news["tian"]); ?></td>
                    <td><?php echo ($news["toop"]); ?></td>

                    <td>
                        <a class="btn" href="<?php echo U('addvip',array('id'=>$news['id']));?>"><span>编辑</span></a>
                        <a class="btn del" href="<?php echo U('vip_delete',array('id'=>$news['id']));?>"><span>删除</span></a>
                    </td>
