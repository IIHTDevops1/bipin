...
<div class="fixed-bar">
    <div class="item-title">
        <h3>入驻商申请列表</h3>
        <ul class="tab-base">
            <li><a href="<?php echo U('gifts_add');?>"><span>添加</span></a></li>
            <!--<?php if(is_array($fen_type)): $i = 0; $__LIST__ = $fen_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?>-->
               <!--<?php if($type == $vv['id']): ?>-->
                   <!--<li><a href="<?php echo U('blackaccount');?>?type=<?php echo ($vv["id"]); ?>" class="current" ><span><?php echo ($vv["cad_name"]); ?>&nbsp;元区</span></a></li>-->
               <!--<?php else: ?>-->
                   <!--<li><a href="<?php echo U('blackaccount');?>?type=<?php echo ($vv["id"]); ?>" ><span><?php echo ($vv["cad_name"]); ?>&nbsp;元区</span></a></li>-->
               <!--<?php endif; ?>-->
            <!--<?php endforeach; endif; else: echo "" ;endif; ?>-->
            <!--<li><a href="<?php echo U('blackaccount');?>?status=1" <?php if(($status) == "1"): ?>class="current"<?php endif; ?> ><span>筹集中</span></a></li>-->
            <!--<li><a href="<?php echo U('blackaccount');?>?status=2" <?php if(($status) == "2"): ?>class="current"<?php endif; ?> ><span>待开奖</span></a></li>-->
            <!--<li><a href="<?php echo U('blackaccount');?>?status=3" <?php if(($status) == "3"): ?>class="current"<?php endif; ?> ><span>已开奖</span></a></li>-->
        </ul>
    </div>
</div>
    ...
    <table class="table tb-type3" id="newslist">
        ...
        <tbody>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$news): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><img src="<?php echo ($news["thumb"]); ?>" style="width: 80px;height: 100px;"></td>
                    <td><?php echo ($news["gifts_title"]); ?></td>
                    <td><?php echo ($news["gold"]); ?></td>
                    <td><?php echo ($news["toop"]); ?></td>
                    <td>
                        <a class="btn" href="<?php echo U('gifts_add',array('id'=>$news['id']));?>"><span>审核通过</span></a>
                        <a class="btn del" href="<?php echo U('gifts_delete',array('id'=>$news['id']));?>"><span>审核不通过</span></a>
                    </td>
